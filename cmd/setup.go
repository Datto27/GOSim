...
checks connectivity, and writes ~/.config/vecsim/config.json.
...
func runSetup(ctx context.Context) error {
	...
	fmt.Println()
	fmt.Println("╔══════════════════════════════════════════╗")
	fmt.Println("║         vecsim — first-run setup         ║")
	fmt.Println("╚══════════════════════════════════════════╝")
	...
	databaseURL := prompt("Database URL", "postgres://vecsim:vecsim@localhost:5432/vecsim?sslmode=disable")
	...
	if _, dbErr := db.Connect(ctx, databaseURL); dbErr != nil {
		...
		fmt.Println("    (Start Postgres with 'docker compose up -d' then run 'vecsim migrate'.)")
	} else {
	...
	fmt.Println("  2. vecsim migrate          (create tables & HNSW index)")
	fmt.Println("  3. vecsim seed --type all  (load 75 seed items)")
	fmt.Println("  4. vecsim index --type all (generate embeddings)")
	fmt.Println("  5. vecsim search \"Inception\" --cross-type")
	fmt.Println("     vecsim serve            (start the HTTP API on localhost:7700)")
	fmt.Println()
